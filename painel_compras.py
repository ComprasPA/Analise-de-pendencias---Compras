        ...
        for comp, col_st in zip(compradores, colunas_st):
            with col_st:
                ...
                if not df_comp_total.empty and 'Status_Detalhado' in df_comp_total.columns:
                    ...
                    # 3. Caixa de Itens Atendidos (retornada à posição original abaixo do backlog)
                    st.markdown(f"""
                    ...
                    # 4. Velocímetros de SLA com espaçamento exato de 30px e escala ajustada para o Emergencial ir exatamente até 4 (completamente evidente)
                    ...
                    fig_emg = go.Figure(go.Indicator(
                        ...
                        number = {'font': {'size': 20, 'color': cor_texto_grafico, 'family': 'Arial Black'}},
                        gauge = {
                            'axis': {'range': [0, 4], 'dtick': 1, 'tickwidth': 1, 'tickcolor': "#475569", 'tickfont': {'size': 11, 'color': cor_texto_grafico, 'family': 'Arial Black'}},
                            'bar': {'color': cor_emg}, 'bgcolor': "rgba(0,0,0,0)", 'borderwidth': 0,
                            'steps': [{'range': [0, 3], 'color': '#2a3b4c' if tema_selecionado != 'Claro' else '#e2e8f0'}, 
                                      {'range': [3, 4], 'color': '#4a2525' if tema_selecionado != 'Claro' else '#fed7d7'}],
